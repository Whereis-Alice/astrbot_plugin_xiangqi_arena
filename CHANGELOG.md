...
- WebUI 普通落子音效新增短促敲击噪声层，并提高中高频与低频叠加音量，让“落子”在非吃子时也更清楚。

## 3.5.2 - 2026-06-03

### Fixed

- 修复 WebUI 验证码以 `0` 开头时，AstrBot 命令参数被转成整数导致前导零丢失、验证命令提示格式错误的问题。
- WebUI 验证命令现在会同时从命令参数、原始消息文本和消息段中兜底提取 6 位验证码。

## 3.5.1 - 2026-06-03

### Fixed

- WebUI 音频解锁改为等待 `AudioContext.resume()` 完成后再播放，减少浏览器拦截导致的无声落子。
- 提高普通落子、吃子和将军合成音效的音量与时长，让反馈更明显。

## 3.5.0 - 2026-06-03

### Added

- WebUI 群聊链接默认进入观战模式，并在页面显示 6 位控制验证码。
- 新增 `棋局验证 123456` 命令，只有生成链接的人发送验证码后，对应浏览器才会获得 WebUI 控制权。
- 新增 `webui_control_code_ttl_seconds` 配置，控制验证码默认 5 分钟有效。

### Changed

- WebUI 控制权绑定到浏览器身份，不会因为同一个链接被验证就让所有打开链接的人都能操作棋局。
- 未验证浏览器会禁用开局、走棋、提示、悔棋和认输，只保留观战与自动刷新。

## 3.4.6 - 2026-06-03

### Changed

- WebUI 普通落子只播放落子音效，不再语音播报。
- WebUI 吃子语音从“吃子”改为更短的“吃”。

## 3.4.5 - 2026-06-03

### Changed

- WebUI 顶部右侧不再显示会话状态和 session 路径，界面更干净。
- WebUI 音效升级为合成音效 + 浏览器中文语音：落子、吃子、将军会分别播报。
- WebUI 吃子同时将军时，会先播放吃子反馈，再补播将军提示。

## 3.4.4 - 2026-06-03

### Changed

- 暗色聊天棋盘 PNG 黑方棋子改为浅棋面、深绿字、深绿外圈，保留黑方识别感并提升可读性。
- WebUI 夜战棋盘同步使用暗绿色黑方文字，不再使用浅色黑方字。

## 3.4.3 - 2026-06-03

### Changed

- 聊天棋盘 PNG 棋子边框收窄，移除会贴进字形的小内圈装饰。
- 聊天棋盘 PNG 四套皮肤的棋子配色整体调浅，减少过深、过重的观感。
- 暗色聊天棋盘 PNG 的棋子底色和文字对比度增强，黑方棋子更容易看清。
- WebUI 夜战棋盘同步提高棋子文字对比度，黑方棋子改为暖浅色文字。
