...
- WebUI 支持玩家落子即时刷新、Bot 后台思考、落子/吃子音效、选中动效和本局时间线
- WebUI 页面主题和棋盘皮肤可分开切换；聊天棋盘 PNG 也可通过配置切换皮肤
...
棋局验证 123456  验证当前浏览器的 WebUI 控制权
...
Bot 会返回一个带 token 的链接。群聊链接默认是观战模式，页面会显示 6 位验证码；只有生成链接的人在原群或私聊里发送：

```text
棋局验证 123456
```

验证通过后，打开该验证码的那个浏览器才可以点击棋子和目标格走棋，也可以在网页里开局、执黑开局、重开、提示、悔棋和认输。其他人打开同一个链接仍然只能观战。验证码默认 5 分钟有效，刷新页面会重新显示可用验证码。

网页端玩家落子会立即刷新棋盘，不需要等 Bot 搜索结束。Bot 思考和台词生成会在后台完成，页面顶部居中显示当前行棋方和回合进度；Bot 落子后网页会自动轮询刷新。浏览器允许音频后，网页会播放落子、吃子、将军音效；吃子和将军会尽量使用浏览器中文语音播报。

网页右上角有两个独立选择：页面主题控制整体明暗，棋盘皮肤控制棋盘本身的经典、青玉、夜战、宣纸风格。网页选择保存在浏览器本地，不影响聊天里发送的棋盘图片；聊天棋盘图片使用插件配置 `board_image_theme`。

网页右侧的棋局记录会把走法和台词合并成时间线，例如：
...
```json
{
  ...
  "webui_control_code_ttl_seconds": 300,
...
如果不用反代而是直接暴露端口，可把 `webui_host` 改成 `0.0.0.0`，并务必用防火墙限制来源。链接 token 默认 1 天过期；插件重启后需要重新发送 `网页下棋` 生成新链接。控制权不会跟着链接共享，而是绑定到通过验证码的浏览器。
...
如果启动后提示“健康检查暂未通过：Cannot connect to host 127.0.0.1:8788”，通常表示服务子进程刚启动但端口还没监听，或服务进程启动后很快退出。低配服务器可以把 `pikafish_http_service_startup_wait_seconds` 调到 `10` 或 `15`；同时发送 `Pikafish服务` 查看最近服务日志。
...
| `webui_control_code_ttl_seconds` | `300` | WebUI 控制验证码有效期；只有链接发起人发送验证码后对应浏览器才能控制 |
| `webui_notify_chat` | `true` | 网页操作是否主动同步回原聊天 |
| `webui_notify_board` | `true` | 网页同步聊天时是否附带棋盘图 |
| `webui_player_name` | `玩家` | WebUI 棋局记录里的玩家显示名 |
| `webui_bot_name` | `爱丽丝` | WebUI 棋局记录里的 Bot 显示名 |
| `webui_player_talk_template` | 多句模板组 | WebUI 棋局记录里玩家每步的默认台词模板；支持多行或 `|` 分隔 |
| `board_image_theme` | `classic` | 聊天棋盘 PNG 皮肤：`classic`、`jade`、`dark`、`paper` |
...
| `pikafish_http_service_startup_wait_seconds` | `6` | 托管服务启动后等待 `/health` 可访问的最长秒数 |
...
- 新增外部引擎失败冷却
- 新增裸坐标走棋
- 新增独立 WebUI，支持点击棋盘下棋、即时刷新、时间线记录并同步聊天
