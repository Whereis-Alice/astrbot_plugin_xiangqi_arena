...
网页端玩家落子会立即刷新棋盘，不需要等 Bot 搜索结束。Bot 思考和台词生成会在后台完成，页面顶部居中显示当前行棋方和回合进度；Bot 落子后网页会自动轮询刷新。浏览器允许音频后，网页会播放落子音效，吃子时会播放更重一点的吃子音效。
...
其中玩家名、Bot 名和玩家默认台词可通过 `webui_player_name`、`webui_bot_name`、`webui_player_talk_template` 配置。
...
```json
{
  ...
  "webui_bot_name": "爱丽丝",
  "webui_player_talk_template": "先这样走一步。"
}
```
...
| `webui_player_talk_template` | `先这样走一步。` | WebUI 棋局记录里玩家每步的默认台词模板 |
