...
- WebUI 支持玩家落子即时刷新、Bot 后台思考、落子/吃子音效、选中动效和本局历史滚动
- WebUI 内置经典、青玉、夜战、宣纸四套皮肤；聊天棋盘 PNG 也可通过配置切换皮肤
...
网页端玩家落子会立即刷新棋盘，不需要等 Bot 搜索结束。Bot 思考和台词生成会在后台完成，页面顶部会显示当前阶段；Bot 落子后网页会自动轮询刷新。浏览器允许音频后，网页会播放落子音效，吃子时会播放更重一点的吃子音效。

网页右上角可以切换皮肤：经典、青玉、夜战、宣纸。这个选择保存在浏览器本地，不影响聊天里发送的棋盘图片；聊天棋盘图片使用插件配置 `board_image_theme`。
